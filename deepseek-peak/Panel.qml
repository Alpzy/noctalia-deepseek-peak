...
// Info-only panel: state, countdown, today's windows, drift warning.
// Settings and the policy checker live elsewhere (Settings.qml, BarWidget.qml);
// this panel only displays state shared through pluginSettings.
Item {
    id: root
    property var pluginApi: null

    // Pulled from pluginSettings on every update(): plain-JS settings objects
    // do not emit change notifiers for QML bindings.
    ...
    // Written by the bar widget's checker (in-memory, shared via pluginSettings).
    ...
    readonly property string countdownText: formatHMS(secondsToNext)

    readonly property int contentPreferredWidth: 320
    readonly property int contentPreferredHeight: Math.round(card.implicitHeight + Style.marginL * 2)
...
// END peak.js (generated)
    ...
    function refreshTexts() {
        ...
        var ci = checkedInfo(nowDate.getTime(), checkedAtMs);
    ...
    function readSettings() {
        var s = (pluginApi && pluginApi.pluginSettings) || {};
        var d = (pluginApi && pluginApi.manifest && pluginApi.manifest.metadata && pluginApi.manifest.metadata.defaultSettings) || {};
        weekendMode = s.weekendMode || d.weekendMode || "beijing-anchored";
        driftColor = s.driftColor || d.driftColor || "#fbbf24";
        drift = s.drift === true;
        checkedAtMs = s.checkedAtMs || 0;
    }
    function update() {
        var now = new Date();
        var wasWeekendMode = weekendMode;
        readSettings();
        if (wasWeekendMode !== weekendMode)
            _ticks = 999;
        nowDate = now;
    ...
    ColumnLayout {
        ...
        NText {
            Layout.fillWidth: true
            text: root.countdownLabel + "  " + root.countdownText
        }
        NDivider {
            Layout.fillWidth: true
        }
